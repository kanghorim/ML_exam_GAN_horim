...
decoded = Dense(784, activation= 'sigmoid' ) # 0~1값 사이라서 시그모이드 사용함
...
decocer_layer = autoencoder.layers[-1]
decocer = Model(encoder_input,decocer_layer(encoder_input))
decocer.summary() # 위에 autoencoder의 디코더 부분
...
fit_hist = autoencoder.fit(flatted_x_train,flatted_x_train,epochs = 50 , batch_size = 256,
                                validation_data = (flatted_x_test, flatted_x_test))

encoded_img = encoder.predict(x_test[:10].reshape(-1,784))
decoded_img = decocer.predict(encoded_img)
